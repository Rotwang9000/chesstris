/**
 * Theme management for Shaktris
 * 
 * Provides functions for managing visual themes for the game.
 */
 ...
 */
export function getTheme(themeName = THEMES.DEFAULT) {
 ...
 */
export function getPieceModelPath(pieceType, themeName = THEMES.DEFAULT) {
	const theme = getTheme(themeName);
	return theme.pieceConfig[pieceType]?.model || themeConfigs[THEMES.DEFAULT].pieceConfig[pieceType].model;
}

/**
 * Get scale for a specific chess piece type in the selected theme
 * @param {string} pieceType - Type of chess piece (king, queen, etc.)
 * @param {string} themeName - Name of the theme
 * @returns {number} Scale factor for the 3D model
 */
export function getPieceScale(pieceType, themeName = THEMES.DEFAULT) {
	const theme = getTheme(themeName);
