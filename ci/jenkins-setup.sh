...
echo "=== Tetches Jenkins CI/CD Setup (Docker) ==="
echo ""
...
for DIR in /var/www/tetches.staging /var/www/tetches.live /var/www/.deploy-triggers; do
...
$CRON_MARKER
* * * * * ${CRON_PM2_PREFIX}$WATCHER_SCRIPT >> /var/log/tetches-deploy.log 2>&1
* * * * * sleep 30 && ${CRON_PM2_PREFIX}$WATCHER_SCRIPT >> /var/log/tetches-deploy.log 2>&1
CRON
) | crontab -
...
sleep 5
INIT_PW=$(docker exec tetches-jenkins cat /var/jenkins_home/secrets/initialAdminPassword 2>/dev/null || echo "")
...
if [ -n "$INIT_PW" ]; then
	...
	echo "  docker exec tetches-jenkins cat /var/jenkins_home/secrets/initialAdminPassword"
fi
...
echo "   - Repository URL: https://github.com/Rotwang9000/tetches.git"
