...
describe('King Capture Mechanics', () => {
	...
	it('should transfer ownership of pieces when a king is captured', () => {
		...
		expect(player1King).toBeDefined();
		expect(player2King).toBeDefined();
		...
		expect(player1PiecesAfter).toBe(initialPlayer1Pieces + initialPlayer2Pieces - 1);
		expect(player2PiecesAfter).toBe(0);
	});
	...
	it('should transfer 50% of fees when a king is captured', () => {
		...
		expect(feeTransfer.from).toBe(player2Id);
		expect(feeTransfer.to).toBe(player1Id);
		expect(feeTransfer.amount).toBe(expectedTransferAmount);
	});
	...
	it('should declare game winner when only one player with a king remains', () => {
		...
		expect(gameWinnerEvent).toBeDefined();
		expect(gameWinnerEvent.winnerId).toBe(player1Id);
		expect(gameWinnerEvent.gameId).toBe(gameId);
		
		// Verify game ended
		expect(game.ended).toBe(true);
		expect(game.winnerId).toBe(player1Id);
		expect(game.endTime).toBeInstanceOf(number);
	});
	...
	it('should not declare a winner if multiple players with kings remain', () => {
		...
		expect(gameWinnerEvent).toBeNull();
