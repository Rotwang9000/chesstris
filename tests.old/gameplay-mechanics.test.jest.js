...
import TetrominoManager from '../server/game/TetrominoManager.js';
import ChessManager from '../server/game/ChessManager.js';
import IslandManager from '../server/game/IslandManager.js';
import PlayerManager from '../server/game/PlayerManager.js';
import { GAME_EVENTS } from '../server/constants.js';
...
describe('Shaktris Gameplay Mechanics', () => {
	...
	describe('Z-axis Tetromino Placement', () => {
		...
		it('should explode tetromino at Z=1 if cell exists underneath', () => {
			...
			expect(game.board[y][x].chessPiece).toBeUndefined();
		});
	});
	...
	describe('Pawn Promotion', () => {
		it('should promote pawn to knight after 8 forward moves', () => {
			...
			// Assert
			expect(result.success).toBe(true);
			expect(result.pawnPromoted).toBe(true);
			expect(game.board[startY-1][startX].chessPiece.type).toBe('knight');
		});
	});
	...
	describe('Row Clearing', () => {
		it('should clear a row when 8 cells are filled', () => {
			...
			// Assert
			expect(clearedRows).toContain(rowIndex);
			expect(game.board[rowIndex][10]).toBeNull();
		});
	...
	describe('King Capture Mechanics', () => {
		it('should transfer captured player pieces to victor', () => {
			...
			expect(capturedPawn.player).toBe(player1Id);
		});
		...
		it('should transfer 50% of fees to victor', () => {
			...
			expect(feeTransfer.amount).toBe(0.75); // 50% of 1.5 SOL
		});
	});
});