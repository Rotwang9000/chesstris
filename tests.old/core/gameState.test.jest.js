...
describe('GameState Module', () => {
	...
	describe('getGameState()', () => {
		it('should return the current game state', () => {
			...
			expect(state).toHaveProperty('fallingPiece');
		});
	});
	...
	describe('updateGameState()', () => {
		it('should update the game state with new values', () => {
			...
			expect(state.currentPlayerId).toBe('123');
		});
	...
	describe('initGameState', () => {
		it('should initialize a new game state with default values', () => {
			...
			expect(state.board.length).toBe(TEST_CONSTANTS.BOARD_HEIGHT);
			expect(state.board[0].length).toBe(TEST_CONSTANTS.BOARD_WIDTH);
			expect(state.players).to.be.an('object');
			expect(state.homeZones).to.be.an('object');
			expect(state.fallingPiece).toBeNull();
		});
		...
		it('should initialize with custom board dimensions', () => {
			...
			expect(state.board.length).toBe(customHeight);
			expect(state.board[0].length).toBe(customWidth);
			expect(state.boardWidth).toBe(customWidth);
			expect(state.boardHeight).toBe(customHeight);
		});
	...
	describe('hasValidCell and isInBounds', () => {
		it('should check if a cell exists in the board', () => {
			...
			// Act & Assert
			expect(gameState.hasValidCell(5, 5)).toBe(false);  // No cell at this position yet
			mockBoard[5][5] = { color: '#ff0000' };  // Add a cell
			expect(gameState.hasValidCell(5, 5)).toBe(true);   // Now there's a cell
			expect(gameState.hasValidCell(-1, 5)).toBe(false); // Out of bounds
		});
		...
		it('should check if coordinates are within board bounds', () => {
			...
			// Act & Assert
			expect(gameState.isInBounds(0, 0)).toBe(true);    // Inside
			expect(gameState.isInBounds(5, 5)).toBe(true);    // Inside
			expect(gameState.isInBounds(-1, 5)).toBe(false);  // Outside (negative x)
			expect(gameState.isInBounds(5, -1)).toBe(false);  // Outside (negative y)
			expect(gameState.isInBounds(TEST_CONSTANTS.BOARD_WIDTH, 5)).toBe(false);  // Outside (max x)
			expect(gameState.isInBounds(5, TEST_CONSTANTS.BOARD_HEIGHT)).toBe(false); // Outside (max y)
		});
	});
	...
	describe('isCellInSafeHomeZone', () => {
		it('should identify cells in a safe home zone', () => {
			...
			// Assert
			expect(gameState.isCellInSafeHomeZone(0, 0, playerId)).toBe(true);
			expect(gameState.isCellInSafeHomeZone(TEST_CONSTANTS.HOME_ZONE_WIDTH - 1, 0, playerId)).toBe(true);
			expect(gameState.isCellInSafeHomeZone(TEST_CONSTANTS.HOME_ZONE_WIDTH, 0, playerId)).toBe(false);
		});
		
		it('should return false for empty home zones', () => {
			expect(gameState.isCellInSafeHomeZone(0, 0, 'nonexistent')).toBe(false);
		});
	});
	...
	describe('generatePlayerColor', () => {
		it('should generate a random color for a player', () => {
			...
			// Assert
			expect(color1).toBeInstanceOf(string);
			expect(color1).to.match(/^#[0-9A-F]{6}$/i);
			expect(color2).toBeInstanceOf(string);
	...
	describe('findHomeZonePosition', () => {
		it('should find a valid position for a new home zone', () => {
			...
			// Assert
			expect(homeZone).toHaveProperty('startX');
			expect(homeZone).toHaveProperty('startY');
			expect(homeZone).toHaveProperty('width', TEST_CONSTANTS.HOME_ZONE_WIDTH);
			expect(homeZone).toHaveProperty('height', TEST_CONSTANTS.HOME_ZONE_HEIGHT);
		});
	...
	describe('createChessPiece', () => {
		it('should create a chess piece with the correct properties', () => {
			...
			expect(piece.id).toBeInstanceOf(string);
	...
	describe('addChessPiecesToHomeZone', () => {
		it('should add chess pieces to a player\'s home zone', () => {
			...
			for (let i = 0; i < player.homeZone.width; i++) {
				...
				expect(mockGameState.board[y][x]).toHaveProperty('piece');
				expect(mockGameState.board[y][x].piece.playerId).toBe(player.id);
			}
		});
	});
	...
	describe('getValidMoves', () => {
		it('should return valid moves for a pawn', () => {
			...
			expect(validMoves[0]).toEqual({ x: pawnPiece.x, y: pawnPiece.y + 1 });
		});
	...
	describe('applyPotionEffect', () => {
		it('should apply potion effect to a player', () => {
			...
			expect(mockGameState.players[playerId].potions[0].type).toBe(potion.type);
			expect(mockGameState.players[playerId].potions[0].value).toBe(potion.value);
			expect(mockGameState.players[playerId].potions[0].expiresAt).toBeInstanceOf(number);
		});
	});
	...
	describe('spawnFallingPiece', () => {
		it('should spawn a falling tetromino piece', () => {
			...
			// Assert
			expect(piece).to.be.an('object');
			expect(piece.type).toBeInstanceOf(string);
			expect(piece.blocks).to.be.an('array');
			expect(piece.color).to.match(/^#[0-9A-F]{6}$/i);
			expect(piece.x).toBeInstanceOf(number);
			expect(piece.y).toBe(0);
			expect(piece.z).toBeInstanceOf(number);
			expect(piece.rotation).toBe(0);
			
			// The piece should also be set in the game state
			expect(mockGameState.fallingPiece).toBe(piece);
		});
	});
	...
	describe('lockFallingPiece', () => {
		it('should lock the falling piece onto the board', () => {
			...
			clearRowsStub.mockReturnValue(0);
			...
			// Assert
			expect(result).toBe(fallingPiece);
			...
			fallingPiece.blocks.forEach(block => {
				...
				expect(mockGameState.board[y][x]).toHaveProperty('color', fallingPiece.color);
			});
			
			// Check that clearFullRows was called
			expect(gameState.clearFullRows).toHaveBeenCalledTimes(1);
		});
	});
	...
	describe('clearFullRows', () => {
		it('should clear full rows and return the number of rows cleared', () => {
			...
			for (let x = 0; x < width; x++) {
				expect(mockGameState.board[5][x]).toBeNull();
			}
		});
	...
	describe('degradeHomeZones', () => {
		it('should degrade empty home zones', () => {
			...
			expect(mockGameState.homeZones[playerId].width).toBe(7); // Reduced by 1
		});
		...
		it('should not degrade home zones with pieces', () => {
			...
			expect(mockGameState.homeZones[playerId].width).toBe(8); // No change
		});
	});
});