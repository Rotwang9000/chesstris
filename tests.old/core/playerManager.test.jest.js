...
// Import test helpers
import { createTestProxy } from '../jest-helpers.js';
const {
	TEST_CONSTANTS,
	createMockBoard,
	createMockGameState,
...
describe('PlayerManager Module', () => {
	...
	beforeEach(() => {
		...
		PlayerManager.getPlayer = jest.fn((playerId) => {
			return mockGameState.players[playerId] || null;
		});
		
		PlayerManager.getAllPlayers = jest.fn(() => {
			return Object.values(mockGameState.players);
		});
		
		PlayerManager.removePlayer = jest.fn((playerId) => {
		...
		PlayerManager.movePiece = jest.fn(async (pieceId, toX, toY, playerId) => {
			const player = mockGameState.players[playerId];
			if (!player) {
	...
	describe('addPlayer', () => {
		it('should add a new player successfully', async () => {
			...
			expect(mockGameState.players[socketId]).toBe(player);
		});
		...
		it('should include user data when provided', async () => {
			...
			expect(player.pieces).to.be.an('array');
			expect(player.score).toBe(0);
			expect(player.userData).toEqual(userData);
	...
	describe('removePlayer', () => {
		it('should remove a player from the game', () => {
			...
			expect(removedPlayer.id).toBe(playerId);
			expect(mockGameState.players[playerId]).toBeUndefined();
		});
	...
	describe('movePiece', () => {
		it('should move a chess piece to a valid position', async () => {
			...
			const getValidMovesStub = jest.fn();
			getValidMovesStub.mockReturnValue(validMoves);
			...
			expect(result.piece).toBe(gameState.players[playerId].pieces[0]);
		...
		it('should handle attack moves correctly', async () => {
			...
			expect(result.capturedPiece).toEqual(targetPiece);
			...
			expect(mockGameState.board['3,3'].piece).toBeUndefined();
			expect(mockGameState.board['5,3'].piece).toBe(mockGameState.players[player1Id].pieces[0]);
		});
		...
		it('should handle potion collection', async () => {
			...
			expect(result).to.be.an('object');
			expect(result.success).toBe(true);
			expect(result.potion).toBe(potion);
			
			// Check that potion was applied
			expect(mockGameState.players[playerId].speed).toBe(3);
			
			// Check that potion was removed from board
			expect(mockGameState.board['5,3'].potion).toBeUndefined();
		});
		...
		it('should return error for invalid move', async () => {
			...
			expect(mockGameState.players[playerId].pieces[0].x).toBe(3);
			expect(mockGameState.players[playerId].pieces[0].y).toBe(3);
		});
	...
	describe('updatePlayerScore', () => {
		...
		it('should update a player\'s score', () => {
			...
			expect(mockGameState.players[playerId].score).toBe(150);
		});
	...
	describe('getWinner', () => {
		...
		it('should return the player with the highest score', () => {
			...
			// Assert
			expect(winner).toEqual(mockGameState.players.player2);
		});
